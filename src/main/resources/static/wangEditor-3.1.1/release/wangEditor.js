...
Video.prototype = {
    ...
    onClick: function onClick() {
        //this._createPanel();
        this._createVideoInsertPanel();
    },

    _createVideoInsertPanel: function _createVideoInsertPanel() {
        var editor = this.editor;
...
// 原型
    UploadVideo.prototype = {
        ...
        uploadVideo: function uploadVideo(files) {
            ...
            var maxSize = 500 * 1024 * 1024;       //500M
            var maxSizeM = maxSize / 1000 / 1000;
            var maxLength = 1;
            var uploadFileName = "video";
            var uploadImgParams = config.uploadImgParams || {};
            var uploadImgParamsWithUrl = config.uploadImgParamsWithUrl;
            var uploadImgHeaders = config.uploadImgHeaders || {};
            var hooks = config.uploadImgHooks || {};
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if (!hooks.customInsert && result.errno != '0') {
                            ...
                            if (hooks.customInsert && typeof hooks.customInsert === 'function') {
                                ...
                                editor.cmd.do('insertHTML', '<video src="' + result.message + '" style="width:100%" controls autobuffer />');
                                _this3._alert("上传视频成功")
                                // hooks.customInsert(_this3.insertLinkImg.bind(_this3), result, editor);
                            } else {
                                // 将图片插入编辑器
                                var data = result.data || [];
